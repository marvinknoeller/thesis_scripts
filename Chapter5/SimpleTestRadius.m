...
lowerrad = 10;
upperrad = 250;
height = 200;%linspace(lowerh * kappa2 * 1e-9, upperh * kappa2 * 1e-9,200);
intrad = 4;% interior radius
eps_rel = geteps_gold(mum);%-5+.1;%-4+0.2*1i;%5;
% Rad = rad * kappa2 * 1e-9;
% Rad = 1.0:0.1:2.2;
Var.kappa = kappa;
M = 11;
...
h = height* kappa2 * 1e-9;
Rad = linspace(lowerrad * kappa2 * 1e-9, upperrad * kappa2 * 1e-9,1000);
parfor ite = 1 : length(Rad)
...
%% 
f=figure;
f.Position = [950 300 353 345];%[950 300 253 345];%[2230 753 334 345];
set(gcf,'color','w');
...
Var.aa = 0.05;
Var.bb = 0.05;
GenerateEllipticScattererNew(p,M,Var.aa,Var.bb,alpha,R,S,T,'gold',0,1);
axis equal
drawnow
% axis off
an = annotation("doublearrow");
an.Position = [0.70594935699559,0.220289855072463,-0.002946675818326,0.657971014492754];

an2 = annotation("doublearrow");
an2.Position = [0.507082152974504,0.176811594202899,0.110481586402266,0.002898550724638];
text(1.232904679460164,0.188903778647013,-0.376284061113859,...%-11.321749505059643,37.301436888059925,435.9649565850114,...
    '$h=200\mathrm{nm}$','FontSize',20,'BackGroundColor','w',...
    'Interpreter','Latex',...
    'Clipping','off')
text(0.058391486936589,-0.08062425610224,-1.985063432008817,...%-11.321749505059643,37.301436888059925,435.9649565850114,...
    '$R$','FontSize',20,'BackGroundColor','w',...
    'Interpreter','Latex',...
    'Clipping','off')
...
filename = "SimpleRadiusObj";
print(gcf,'-depsc',filename);

%%
f=figure;
f.Position = [2230 753 334 345];
plot(Rad/kappa2 * 1e9,chir,'--k',Rad/kappa2 * 1e9,smooth_relax...
    ,'-b','LineWidth',2);
xlim([lowerrad,upperrad]);
ylim([0,.6])
ell = legend('$X_{2,2}$','$X_{\rm{HS},2}$','Interpreter','Latex',...
    'Fontsize',20,'Location','NorthEast');
set(gca,'FontSize', 16);
xlabel('radius $R$ in $\mathrm{nm}$','Interpreter','Latex','Fontsize',14);
...
plot(Rad/kappa2 * 1e9,full_der...
    ,'-b','LineWidth',2);
% hold on 
% plot(Rad/kappa2 * 1e9,numapp,'-r','LineWidth',2)
xlim([lowerrad,upperrad]);


ell = legend("$X_{\rm{HS},2}'$",'Interpreter','Latex',...
    'Fontsize',20,'Location','NorthEast');
% ylim([0,1])
