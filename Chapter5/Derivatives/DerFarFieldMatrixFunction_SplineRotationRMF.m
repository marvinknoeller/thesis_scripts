function DerFarFieldMatrix = DerFarFieldMatrixFunction_SplineRotationRMF(p,Var,hroh,R,S,T)
%% preambel
% get the constants
eps_rel = Var.eps_rel;
mu_rel = Var.mu_rel;
...
[~,~,coefsh,~,tsh] = splinepoints(hroh,M,ts);
[h_in_between,~,~,~] = allpoints(coefsh,tsh,num_x,M);

for ii = 1:length(tt)
    DDreal(:,:,ii) = [real(const_eps1), 0; 0 real(const_eps2)];
    DDimag(:,:,ii) = [imag(const_eps1), 0; 0 imag(const_eps2)];
    
    DD2real(:,:,ii) = [real(const_mu1), 0; 0 real(const_mu2)];
    DD2imag(:,:,ii) = [imag(const_mu1), 0; 0 imag(const_mu2)];
end
%%
...
for ki = 1:size(p_in_between,2)
    ...
    VVV = [tan, nor, bnor];
    WWW = [tan, nor, bnor];
    DerVVV = [zeros(3,1), h_in_between(ki) * S(:,ki), -h_in_between(ki)*R(:,ki)];
    DerWWW = [zeros(3,1), h_in_between(ki) * S(:,ki), -h_in_between(ki)*R(:,ki)];
    DerPol_1(:,:,ki) = DerVVV*diag([1,DD2real(1,1,ki),DD2real(2,2,ki)])*((VVV).') ...
        + VVV*diag([1,DD2real(1,1,ki),DD2real(2,2,ki)])*((DerVVV).')...
        +1i * (DerWWW*diag([0,DD2imag(1,1,ki),DD2imag(2,2,ki)])*(WWW.')...
        + WWW*diag([0,DD2imag(1,1,ki),DD2imag(2,2,ki)])*(DerWWW.'));
    DerPol_2(:,:,ki) = DerVVV*diag([1,DDreal(1,1,ki),DDreal(2,2,ki)])*((VVV).')...
        + VVV*diag([1,DDreal(1,1,ki),DDreal(2,2,ki)])*((DerVVV).')...
        +1i * (DerWWW*diag([0,DDimag(1,1,ki),DDimag(2,2,ki)])*(WWW.')...
        + WWW*diag([0,DDimag(1,1,ki),DDimag(2,2,ki)])*(DerWWW.'));
end
...
HH1 = Tensor_Mat_Mult(DerPol_1,Inc1);
HH2 = Tensor_Mat_Mult(permute(Scal1,[2,1,3]),HH1);
Help1 = (mu_rel-1)*HH2;

II1 = Tensor_Mat_Mult(DerPol_2,Inc2);
...
DerFarFieldMatrix = sum((wvec).*integrand,3);