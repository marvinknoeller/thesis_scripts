...
for number = 1:length(radii)
    name = strcat('farfields/metallic21/dhelix/farField_DHelixMetal21',...
        radii(number),'.mat');
    load(name)
    E_Bempp= FarField;%
    t=linspace(0,1,n);
...
set(gcf,'color','w')
loglog(rad_plot,normi,'-db',rad_plot,rad_plot.^2*3e-4,'--r','LineWidth',2,'MarkerSize',8)
hold on
...
xlim(round(1/alpha * 1e9 * [0.03,0.2],2));
ylim([1.0000e-03 1]);
xticks (round(1/alpha * 1e9 * [0.03 0.05 0.07 0.1 0.15 0.2],2))
grid on
ax = gca;
ax.GridAlpha = 0.8;
...
line([5.348 8.021],[0.0085,0.0085],'Color','r','LineWidth',2)
line([8.021 8.021],[0.0085,0.019],'Color','r','LineWidth',2)
text(8.79603295266102,0.013424259241547,0,"slope 2",'Interpreter','Latex','FontSize',20,'BackgroundColor', 'w','EdgeColor',...
    'k','LineWidth',1.)
line([5.348, 8.021],[0.037,0.037],'Color','m','LineWidth',2)
line([8.021, 8.021],[0.037, 0.056],'Color','m','LineWidth',2)
text(8.79603295266102,0.042949483100216,0,"slope 1",'Interpreter','Latex','FontSize',20,'BackgroundColor', 'w','EdgeColor',...
    'k','LineWidth',1.)
axis square
filename='Ringplots/Plot3M21';
